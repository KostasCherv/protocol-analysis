        ...
        if trace and "result" in trace:
            ...
            if isinstance(result, dict):
                ...
                if "output" in result:
                    ...
                    if output and output.startswith("0x08c379a0"):  # Error(string) selector
                        # Decode error string
                        try:
                            ...
                            error_str = bytes.fromhex(str_data_hex).decode('utf-8').rstrip('\x00')
                            return error_str
                        except:
                            pass
...
def get_account(account_index: Optional[int] = None, private_key: Optional[str] = None) -> Account:
...
def execute_multicall(
    ...
    for call in calls:
        ...
        call_data_hex = "0x" + call_data.hex() if isinstance(call_data, bytes) else call_data
    ...
    try:
        ...
        for log in receipt.logs:
            if len(log.topics) > 0 and log.topics[0] == event_sig:
                credit_account = w3.to_checksum_address("0x" + log.topics[1].hex()[-40:])
                break
        
        if not credit_account:
            return {"success": False, "error": "Could not extract credit account from event"}
    ...
    try:
        ...
        if isinstance(e, tuple):
            ...
            if len(e.args) == 1:
                ...
                if isinstance(e.args[0], str) and e.args[0].startswith("0x") and len(e.args[0]) == 10:
